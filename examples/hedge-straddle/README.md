...
- **End-to-end pending-order lifecycle on MT5.** Phase 26b added native MT5 translation for `Stop`, `Limit`, `StopLimit`, `StandaloneOCO`, and `TrailingStop` — placements now route to MT5 as `BUY_STOP` / `SELL_STOP` / `BUY_STOP_LIMIT` / etc. wire requests, with OCO legs tagged with a shared `oco:<id>` comment. However, the fill-event lifecycle (qkt detecting that a pending order has filled, and triggering OCO sibling cancellation) currently relies on the position poller and may lag. The dedicated pending-order poller lands in Phase 26c.
