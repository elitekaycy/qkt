...
usage() {
    cat <<'EOF'
Usage: run-readonly-gateway-restart.sh --scenario DIR --gateway-container NAME [--cli PATH]
       run-readonly-gateway-restart.sh --scenario DIR --gateway-container NAME [--cli PATH] --verify-only

Verifies a prepared live-validation scenario, runs the prepared read-only strategy against the real localhost MT5 gateway, waits for exact pre-restart M1/M5 evidence, restarts the named Docker gateway container, proves feed disconnect and reconnect, then requires exact post-restart M1/M5 evidence again with the account still flat and unchanged.
EOF
}

fail() {
    printf 'run-readonly-gateway-restart: %s\n' "$1" >&2
    exit 1
}
    ...
    for attempt in 1 2 3; do
        ...
        if [ "$delay_ms" -eq 0 ]; then
            jq -n \
                ...
                {
                  schema:"qkt-live-readonly-gateway-restart-startup-window-v1",
                  status:"passed",
        ...
        [ "$((total_wait_seconds + sleep_seconds))" -le "$max_total_wait_seconds" ] ||
            fail "broker tick clock did not enter the gateway-restart startup window within 260 seconds"
        total_wait_seconds=$((total_wait_seconds + sleep_seconds))
        sleep "$sleep_seconds"
    done
    fail "broker tick clock did not enter the bounded gateway-restart startup window after three observations"
}
...
wait_for_status_running() {
    ...
    fail "daemon did not load the read-only strategy cleanly"
}
...
wait_for_status_running "$evidence/daemon-status-initial.json"
...
jq -e 'length == 0' "$evidence/positions-final.json" >/dev/null || fail "read-only gateway-restart run ended with an open position"
jq -e 'length == 0' "$evidence/orders-final.json" >/dev/null || fail "read-only gateway-restart run ended with a pending order"
jq -e 'length == 0' "$evidence/history-during-run.json" >/dev/null || fail "read-only gateway-restart run unexpectedly produced a venue deal"
jq -e --slurpfile initial "$evidence/gateway-account-initial.json" '
    ...
    .trade_expert == true
' "$evidence/gateway-account-final.json" >/dev/null || fail "read-only gateway-restart run changed the venue account snapshot"
...
[ "$order_events" -eq 0 ] || fail "read-only gateway restart emitted an order, fill, accounting, linkage, or rejection event"
...
pre_1m_matches="$(qkt_count_matched_evaluations "$strategy_name" eur1 EXNESS:EURUSD 1m "$session_started_ms" "$restart_started_ms" "${audit_journals[@]}")"
pre_5m_matches="$(qkt_count_matched_evaluations "$strategy_name" eur5 EXNESS:EURUSD 5m "$session_started_ms" "$restart_started_ms" "${audit_journals[@]}")"
...
pre_1m_warmups="$(qkt_count_warmup_pseudo_ticks EXNESS:EURUSD 60000 "$daemon_started_ms" "$restart_started_ms" "${audit_journals[@]}")"
pre_5m_warmups="$(qkt_count_warmup_pseudo_ticks EXNESS:EURUSD 300000 "$daemon_started_ms" "$restart_started_ms" "${audit_journals[@]}")"
...
[ "$gateway_mutations" -eq 0 ] || fail "read-only gateway restart issued a mutating gateway request"
...
[ "$disconnect_warnings" -gt 0 ] || fail "gateway restart retained no feed disconnect warning"
[ "$reconnect_infos" -gt 0 ] || fail "gateway restart retained no feed reconnect info"
