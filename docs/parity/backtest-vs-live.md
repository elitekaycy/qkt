...
| A27 | `STACK_AT` leg protection was anchored on the tick mark (the mid on MT5 metals and FX) and never re-anchored on the leg's fill. A market BUY fills at the ask, so a leg's target sat half a spread closer than declared, and a target smaller than half the spread landed on the wrong side of the entry. Measured live on Exness XAUUSD (0.26 spread): a `TAKE PROFIT BY 0.10` tier fired 0.12 s after the venue's first qualifying tick and all 10 legs were rejected (`For BUY orders, TP must be above entry price`, TP 4320.306 against an ask of 4320.399). The mt5-sim replay of the same ticks OPENED those legs, with the target below the fill | FIXED — a firing tier anchors its bracket at the ask (BUY) or bid (SELL) and carries its `BY` distances, so the leg re-anchors on its actual fill in both modes, like a primary bracket. Excursion stays measured on the mark (`StackEngineTest`, `TradingPipelineStackFillAnchorTest`) |
| A28 | Submit-time protection was validated against the mid and only for absolute `AT` targets, while the MT5 gateway (`validate_sl_tp`) validates against the entry's execution price (ask for a market BUY, bid for a market SELL) and validates whatever target the order carries — on an attach venue that is a `BY`/`PCT`/`RR` target's pre-fill placeholder. So a backtest filled brackets live refused, and refused a BUY stop between the mid and the ask that live accepted | FIXED — `VenueSubmission` checks the submitted SL and TP against the execution quote (or the pending entry's own price) with the gateway's strict inequalities and message text, in both modes (`OrderManagerBracketCrossedProtectionTest`). A primary `BY` target smaller than half the spread is now refused in backtest exactly as live refuses it |
