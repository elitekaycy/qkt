...
| A19 | Resting limit ladders: when one replayed tick gaps through several resting levels at once, replay fills each of them at that tick's price and can fill a level live never reached. Measured on a ten-level 2-point ladder: live filled 4 legs at their own levels (…247/…245/…243/…241), replay filled 5, two of them at the same price and the same timestamp. The seed also filled 2 points higher in replay, and because layer triggers anchor to the seed fill the whole ladder shifts with it | OPEN — affects pending/limit ladders, not market bursts (`TIMES`/burst entries reproduce exactly). Compare live against `mt5-sim` with `scripts/live-validation/compare-stack-replay.sh` before trusting a ladder backtest |
