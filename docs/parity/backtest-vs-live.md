...
| A22 | `max_trades_per_day` counts entry FILLS, so a burst outruns it live for the same reason A21 did: the whole burst is risk-checked before any of it fills, and every order reads the same pre-burst total. Measured on the demo account: a strategy capped at 60 executed 100 in one `TIMES 100` burst live with zero rejections, while the backtest stopped it at 60 | OPEN — the obvious fix (count in-flight entries, as A21 now does) was attempted and reverted: it changed the backtest's own count in a way not yet fully explained, and a half-understood change to a risk rule is worse than a documented gap. Until it is resolved, cap burst depth explicitly in the strategy rather than relying on this limit |
| A23 | `book_risk.max_gross_exposure` is inert for a standalone strategy: `BookRiskController` is constructed only by `PortfolioDeployer`, so a single-strategy deployment silently ignores the block. Measured: a 100-leg EURUSD burst reached roughly 1.16x account capital in notional against a declared `max_gross_exposure: 0.60`, with no rejection | OPEN — there is no enforced aggregate-exposure cap for a standalone strategy. `max_position_size` is per-symbol and `max_open_positions` caps distinct symbols; neither bounds total notional. Deploy burst strategies as a portfolio to get book risk, or bound exposure inside the strategy |
