# Phases

qkt is built in numbered phases. Each phase has a post-merge changelog with usage examples, migration notes, and known limitations.

## Recent

- [Phase 32 — bid/ask DSL exposure](phase-32-bid-ask.md)
- [Phase 31 — Telegram alerts](phase-31-telegram-alerts.md)
- [Phase 30 — Instrument metadata](phase-30-instrument-metadata.md)
- [Phase 29 — Engine state persistence](phase-29-engine-state-persistence.md)
- [Phase 28 — Multi-source market data](phase-28-multi-source-marketdata.md)
- [Phase 27 — Conditional bracketed stacks (`STACK_AT`)](phase-27-conditional-bracketed-stacks.md)
- [Phase 26d — `/orders` poller, PERCENT trailing, order modification](phase-26d-orders-percent-modify.md)
- [Phase 26c — Pending-order fill-event lifecycle on MT5](phase-26c-pending-fill-lifecycle.md)
- [Phase 26b — MT5 native pending family + OCO + trailing](phase-26b-mt5-pending-family.md)
- [Phase 26a — Pending-entry OCO DSL surface and clock accessors](phase-26a-pending-oco-and-clock.md)
- [Phase 25B — Live auto-warmup on deploy](phase-25b-live-auto-warmup.md)
- [Phase 24 — Risk-sizing primitives](phase-24-risk-sizing-primitives.md)
- [Phase 23 follow-up — DSL corrections and packaging](phase-23-followup.md)
- [Phase 23 — DSL catalog expansion](phase-23.md)
- [Phase 22 — KDoc the public API](phase-22.md)
- [Phase 21 — Documentation site](phase-21.md)
- [Phase 20 — Quickstart + docker compose](phase-20.md)
- [Phase 19 — Pre-live confidence pack](phase-19.md)
- [Phase 18 — LiveSession typed-broker dispatch](phase-18.md)
- [Phase 17 — MT5 broker](phase-17.md)
- [Phase 16 — Backtest fidelity + HTML report](phase-16.md)
- [Phase 15 — DSL `LOG`](phase-15.md)
- [Phase 14 — Portfolio v2](phase-14.md)

## Phase 13 — STACK

- [Phase 13a — STACK keyword](phase-13a-stack.md)
- [Phase 13b](phase-13b.md)

## Phase 12 — CLI + observability

- [Phase 12a — CLI](phase-12a-cli.md)
- [Phase 12b — Observability](phase-12b-observability.md)
- [Phase 12c — Daemon](phase-12c-daemon.md)

## Phase 11 — DSL

- [Phase 11a — OSS baseline](phase-11a-oss-baseline.md)
- [Phase 11b — DSL foundation](phase-11b-dsl-foundation.md)
- [Phase 11c1 — State and operators](phase-11c1-state-and-operators.md)
- [Phase 11c2 — Snapshots and aggregates](phase-11c2-snapshots-aggregates.md)
- [Phase 11c3 — Actions](phase-11c3-actions.md)
- [Phase 11d1 — Order surface](phase-11d1-order-surface.md)
- [Phase 11d2 — Equity defaults](phase-11d2-equity-defaults.md)
- [Phase 11e — Multistream](phase-11e-multistream.md)
- [Phase 11f — Parser](phase-11f-parser.md)

## Phase 10 — Backtest reporting

- [Phase 10 — Backtest reporting](phase-10-backtest-reporting.md)
- [Phase 10b — Parameter sweep](phase-10b-parameter-sweep.md)
- [Phase 10c — Walk-forward](phase-10c-walk-forward.md)

## Phase 9 — Risk

- [Phase 9 — Risk engine](phase-9-risk-engine.md)

## Phase 8 — Strategy context

- [Phase 8 — Strategy context and P&L attribution](phase-8-strategy-context-and-pnl-attribution.md)

## Foundations

- [Phase 1 — Core engine MVP](phase-1-engine.md)
- [Phase 2a — Event bus + SLF4J](phase-2a-event-bus.md)
- [Phase 2b — Candle aggregator](phase-2b-candles.md)
- [Phase 3 — Risk engine + P&L](phase-3-risk-pnl.md)
- [Phase 4 — Backtest harness](phase-4-backtest.md)
- [Phase 5 — Indicator catalog](phase-5-indicators.md)
- [Phase 6 — Historical data layer](phase-6-historical-data.md)

## Phase 7 — Live runtime

- [Phase 7 — Live runtime](phase-7-live-runtime.md)
- [Phase 7d — Broker and orders](phase-7d-broker-and-orders.md)
- [Phase 7e — Bybit and composite](phase-7e-bybit-and-composite.md)
- [Phase 7f — Broker resilience](phase-7f-broker-resilience.md)
- [Phase 7g — Reconciliation and balances](phase-7g-reconciliation-and-balances.md)
- [Phase 7h — Derivatives and rate limit](phase-7h-derivatives-and-rate-limit.md)
