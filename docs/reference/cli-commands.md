...
| `qkt golden capture --session <strategy> [--state-dir DIR] [--out ZIP]` | Export retained live ticks, warmup ticks, completed candles, fills, orders, and raw MT5 exchanges as a checksummed ZIP. Market records must contain structured replay data. The command requires a filled order linked to a successful MT5 `/order` exchange by explicit engine ID or venue ticket and fails if evidence is missing or a journal reports dropped records. Manifest `capture*` build fields identify the CLI that created the ZIP; a supervising run manifest must separately identify the daemon build that produced the session. |
