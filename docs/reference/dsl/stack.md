...
- **A resting ladder does not backtest exactly.** Layer triggers anchor to the seed fill, so a seed that fills a couple of points away in replay shifts every level with it, and a single replayed tick that gaps through several levels can fill more of them than the live venue did. Market bursts do not have this problem. Compare a ladder against a live capture before trusting its backtest (catalog row A19).
