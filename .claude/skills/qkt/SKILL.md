...
- **Ease of use.** Public API and DSL must read cold: KDoc plus one worked example, the smallest surface that does the job. If a caller has to learn a new primitive, justify it.

This is judgment, not a gate — a docs typo needs no latency analysis. But a hot-path or public-API change that silently ignores one of these axes is incomplete.
