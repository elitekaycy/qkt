...
class BookAnalyticsIntegrationTest {
    ...
    fun `two-strategy backtest produces book analytics keyed by strategy`() {
        ...
        assertThat(ba!!.contributionToReturn.keys).containsExactlyInAnyOrder("alpha", "beta")
        assertThat(ba.riskContribution.keys).containsExactlyInAnyOrder("alpha", "beta")
        val sum = ba.contributionToReturn.values.fold(BigDecimal.ZERO) { s, v -> s.add(v) }
    ...
    fun `single-strategy backtest has null book analytics`() {
        ...
        assertThat(result.bookAnalytics).isNull()
    }
}
