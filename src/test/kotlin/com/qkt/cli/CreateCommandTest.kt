...
class CreateCommandTest {
    ...
    fun `--kind bybit scaffolds the no-gateway tree wired for Bybit REST`(
        ...
        val accounts = Config.load(target.resolve("qkt.config.yaml")).accountConfigs()
        assertThat(accounts.map { it.name to it.type })
            .withFailMessage("bybit config should declare one account per category the strategies use")
            .containsExactly("bybit_spot" to "bybit", "bybit_linear" to "bybit")
        val opened = TestAccounts.directory(*accounts.toTypedArray())
