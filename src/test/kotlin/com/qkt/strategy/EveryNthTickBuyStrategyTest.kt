...
class EveryNthTickBuyStrategyTest {
    ...
    fun `emits no signal on first n minus 1 ticks`() {
        ...
        repeat(9) { strategy.onTick(tick(), testStrategyContext()) { signals.add(it) } }
    ...
    fun `emits Buy on the nth tick`() {
        ...
        repeat(10) { strategy.onTick(tick(), testStrategyContext()) { signals.add(it) } }
    ...
    fun `emits Buy on every nth tick`() {
        ...
        repeat(15) { strategy.onTick(tick(), testStrategyContext()) { signals.add(it) } }
    ...
    fun `ignores ticks for non-target symbol`() {
        ...
        strategy.onTick(tick("EURUSD"), testStrategyContext()) { signals.add(it) }
        strategy.onTick(tick("GBPUSD"), testStrategyContext()) { signals.add(it) }
        assertThat(signals).isEmpty()
    }
