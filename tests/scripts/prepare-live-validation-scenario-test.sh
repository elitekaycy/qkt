...
loss_streak_reentry_out="$tmp/loss-streak-reentry-scenario"
bash "$script" \
    --output "$loss_streak_reentry_out" \
    --id validation_loss_streak_reentry \
    --gateway-url http://127.0.0.1:5001 \
    --expected-login 436804390 \
    --expected-server Exness-MT5Trial9 \
    --expected-balance 100000.22 \
    ...
    --magic 917013 \
    --symbol XAUUSD \
    --lifecycle reentry_blocked_loss_streak >/dev/null
loss_streak_reentry_armed="$loss_streak_reentry_out/strategies/armed/validation_loss_streak_reentry_market_bracket.qkt"
[ "$(grep -Fc 'TRADES.today < 2' "$loss_streak_reentry_armed")" -eq 2 ]
[ "$(grep -Fc 'TRADES.today = 0' "$loss_streak_reentry_armed")" -eq 0 ]
grep -F 'max_trades_per_day: 2' "$loss_streak_reentry_out/qkt.config.yaml" >/dev/null
grep -F 'loss_streak_halt: "1"' "$loss_streak_reentry_out/qkt.config.yaml" >/dev/null
grep -F 'loss_streak_halt_scope: persistent' "$loss_streak_reentry_out/qkt.config.yaml" >/dev/null
jq -e '
    .safety.maximumTradesPerDay == 2 and
    .armedScenario.lifecycle == "reentry_blocked_loss_streak" and
    .armedScenario.maximumEntries == 1 and
    .armedScenario.maximumExits == 1 and
    .armedScenario.maximumBlockedEntries == 1 and
    .armedScenario.expectedBlockedReason == "LossStreakHalt" and
    (.armedScenario.closeWhen | contains("second entry intentionally blocked by LossStreakHalt"))
' "$loss_streak_reentry_out/expected.json" >/dev/null
"$cli" parse "$loss_streak_reentry_armed" >/dev/null
(cd "$loss_streak_reentry_out" && sha256sum --check SHA256SUMS >/dev/null)

next_day_reentry_out="$tmp/next-day-reentry-scenario"
bash "$script" \
    --output "$next_day_reentry_out" \
    --id validation_next_day_reentry \
    --gateway-url http://127.0.0.1:5001 \
    --expected-login 436804390 \
    --expected-server Exness-MT5Trial9 \
    --expected-balance 100000.22 \
    ...
    --magic 917014 \
    --symbol XAUUSD \
    --lifecycle reentry_max_trades_next_day_recovered >/dev/null
next_day_reentry_armed="$next_day_reentry_out/strategies/armed/validation_next_day_reentry_market_bracket.qkt"
next_day_seed="$next_day_reentry_out/state/state/validation_next_day_reentry_market_bracket/risk-state.json"
[ "$(grep -Fc 'TRADES.today < 2' "$next_day_reentry_armed")" -eq 2 ]
[ "$(grep -Fc 'TRADES.today = 0' "$next_day_reentry_armed")" -eq 0 ]
grep -F 'max_trades_per_day: 1' "$next_day_reentry_out/qkt.config.yaml" >/dev/null
test -f "$next_day_seed"
jq -e '
    .safety.maximumTradesPerDay == 1 and
    .armedScenario.lifecycle == "reentry_max_trades_next_day_recovered" and
    .armedScenario.maximumEntries == 1 and
    .armedScenario.maximumExits == 1 and
    .armedScenario.maximumBlockedEntries == 1 and
    .armedScenario.expectedBlockedReason == "MaxTradesPerDay" and
    .armedScenario.seededRiskState.kind == "previous-day-max-trades" and
    .armedScenario.seededRiskState.strategy == "validation_next_day_reentry_market_bracket" and
    .armedScenario.seededRiskState.path == "state/state/validation_next_day_reentry_market_bracket/risk-state.json" and
    (.armedScenario.closeWhen | contains("previous-day max-trades seed is ignored"))
' "$next_day_reentry_out/expected.json" >/dev/null
jq -e '
    .version == 1 and
    .strategyId == "validation_next_day_reentry_market_bracket" and
    .halted == false and
    .haltReason == null and
    (.pacerEntryFillsByStrategy.validation_next_day_reentry_market_bracket | length) == 1
' "$next_day_seed" >/dev/null
jq -e --slurpfile expected "$next_day_reentry_out/expected.json" '
    .epochDay == $expected[0].armedScenario.seededRiskState.epochDay and
    .pacerEntryFillsByStrategy.validation_next_day_reentry_market_bracket[0] == $expected[0].armedScenario.seededRiskState.entryFillMs
' "$next_day_seed" >/dev/null
"$cli" parse "$next_day_reentry_armed" >/dev/null
(cd "$next_day_reentry_out" && sha256sum --check SHA256SUMS >/dev/null)

daily_halt_next_day_out="$tmp/daily-halt-next-day-scenario"
bash "$script" \
    --output "$daily_halt_next_day_out" \
    --id validation_daily_halt_reset \
    --gateway-url http://127.0.0.1:5001 \
    --expected-login 436804390 \
    --expected-server Exness-MT5Trial9 \
    --expected-balance 100000.22 \
    ...
    --magic 917015 \
    --symbol XAUUSD \
    --lifecycle reentry_daily_halt_next_day_recovered >/dev/null
daily_halt_next_day_armed="$daily_halt_next_day_out/strategies/armed/validation_daily_halt_reset_market_bracket.qkt"
daily_halt_seed="$daily_halt_next_day_out/state/state/validation_daily_halt_reset_market_bracket/risk-state.json"
[ "$(grep -Fc 'TRADES.today < 2' "$daily_halt_next_day_armed")" -eq 2 ]
[ "$(grep -Fc 'TRADES.today = 0' "$daily_halt_next_day_armed")" -eq 0 ]
grep -F 'max_trades_per_day: 1' "$daily_halt_next_day_out/qkt.config.yaml" >/dev/null
test -f "$daily_halt_seed"
jq -e '
    .safety.maximumTradesPerDay == 1 and
    .armedScenario.lifecycle == "reentry_daily_halt_next_day_recovered" and
    .armedScenario.maximumEntries == 1 and
    .armedScenario.maximumExits == 1 and
    .armedScenario.maximumBlockedEntries == 1 and
    .armedScenario.expectedBlockedReason == "MaxTradesPerDay" and
    .armedScenario.seededRiskState.kind == "previous-day-daily-halt" and
    .armedScenario.seededRiskState.strategy == "validation_daily_halt_reset_market_bracket" and
    .armedScenario.seededRiskState.path == "state/state/validation_daily_halt_reset_market_bracket/risk-state.json" and
    .armedScenario.seededRiskState.haltScope == "DAILY" and
    .armedScenario.seededRiskState.haltReason == "DailyLoss" and
    (.armedScenario.closeWhen | contains("previous-day DAILY risk halt is ignored"))
' "$daily_halt_next_day_out/expected.json" >/dev/null
jq -e '
    .version == 1 and
    .strategyId == "validation_daily_halt_reset_market_bracket" and
    .halted == true and
    .haltReason == "DailyLoss" and
    .haltScope == "DAILY" and
    (.strategyHalts | length) == 1 and
    .strategyHalts[0].strategyId == "validation_daily_halt_reset_market_bracket" and
    .strategyHalts[0].scope == "DAILY" and
    (.pacerEntryFillsByStrategy.validation_daily_halt_reset_market_bracket // [] | length) == 0
' "$daily_halt_seed" >/dev/null
jq -e --slurpfile expected "$daily_halt_next_day_out/expected.json" '
    .epochDay == $expected[0].armedScenario.seededRiskState.epochDay and
    .haltEpochDay == $expected[0].armedScenario.seededRiskState.epochDay and
    .strategyHalts[0].epochDay == $expected[0].armedScenario.seededRiskState.epochDay
' "$daily_halt_seed" >/dev/null
...
bash "$repo_root/scripts/live-validation/run-market-bracket.sh" \
    ...
    --verify-only >/dev/null
bash "$repo_root/scripts/live-validation/run-market-bracket.sh" \
    --scenario "$loss_streak_reentry_out" \
    --cli "$cli" \
    --verify-only >/dev/null
bash "$repo_root/scripts/live-validation/run-market-bracket.sh" \
    --scenario "$next_day_reentry_out" \
    --cli "$cli" \
    --verify-only >/dev/null
bash "$repo_root/scripts/live-validation/run-market-bracket.sh" \
...
grep -F 'reentry_blocked_loss_streak' "$repo_root/scripts/live-validation/run-market-bracket.sh" >/dev/null
...
grep -F '$lifecycle == "reentry_blocked_loss_streak"' "$comparison_script" >/dev/null
...
grep -F 'Loss-streak halt is a live pacing-state halt' "$comparison_script" >/dev/null
